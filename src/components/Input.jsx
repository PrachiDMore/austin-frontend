...
const Input = ({step=1, id, label = false, placeholder, onChange, value, type, password = false, required = false, readOnly = false, className }) => {
  ...
  return (
    <>
      <div className={'flex flex-col Nunito relative ' + className}>
        ...
        {password ? <input required={required} onChange={onChange} value={value} readOnly={readOnly} className='border focus:shadow-purpleShadow duration-300 outline-none shadow-md px-4 py-3 rounded-md w-full ' id={id} type={showPassword ? "text" : "password"} placeholder={placeholder} /> : type === "time" ? <input onChange={onChange} readOnly={readOnly} value={value} className='read-only:cursor-pointer border focus:shadow-purpleShadow duration-300 outline-none shadow-md px-4 py-3 rounded-md w-full ' id={id} type={"time"} placeholder={placeholder} required={required} /> : <input step={step} onChange={onChange} readOnly={readOnly} value={value} className='read-only:cursor-pointer border focus:shadow-purpleShadow duration-300 outline-none shadow-md px-4 py-3 rounded-md w-full ' id={id} type={type} placeholder={placeholder} required={required} />}
